...
// Implementation of bounding window calcualtor:
...
// Function for deleting vector elements from containing vector:
...
// Wrapper for LIBLINEAR's prediction function:
...
// Function to extract LBP feature vectors:
			...
			for(it = imagePyramid.begin(); it != imagePyramid.end(); ++it){
				...
				for(unsigned int r = 0; r < image.rows; r += stride){
					...
					for(unsigned int c = 0; c < image.cols; c += stride){
						...
						vector<double> featureVector = extractLBPFeatureVector(imageRegion, 5, 2, 8, "u2", true);
...
// Function implementing non-maxima suppression:
...
void svmDetect(ctx* context, Size windowSize, double scaling, int stride, bool nms, int minNeighbours, double overlapThresh){
	...
	const char *MODEL_FILE = "lbp-u2-4500-train.scale.model";
...
// Function for creating SVM training files:
