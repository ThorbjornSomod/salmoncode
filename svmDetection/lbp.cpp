...
LBP & LBP::calcLBP(Mat d_img, double radius, bool borderCopy){
	...
	for(int i = 0; i < samples; i++){
		...
		if((fabs(x - rx) < 1e-6) && (fabs(y - ry) < 1e-6)){
			...
			addWeighted(d_img(Rect(fx, fy, dx, dy)), w1, d_img(Rect( cx, fy, dx, dy)), w2, 0, N);
	...
	if(type != LBP_MAPPING_NONE){
		...
		MatIterator_<unsigned char> it = result.begin<unsigned char>(), it_end = result.end<unsigned char>();
...
void LBP::initHF(void){
	...
	fftPlan = fftw_plan_dft_r2c_1d(fftN, fftIn, reinterpret_cast<fftw_complex *>(fftOut), FFTW_ESTIMATE);
